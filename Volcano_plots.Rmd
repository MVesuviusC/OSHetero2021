...
test <- sapply(trouble_shoot, function(x) {
    x$geneID[1] %>%
        str_split(pattern = "/") %>% as.vector()
    })
ggvenn::ggvenn(test)

met_genes <- unlist(test) %>% unique()

os17_cx_test <- AddModuleScore(os17_cx,
                               features = list(met_genes = met_genes),
                               name = "met_genes")
FeaturePlot(os17_cx_test, features = "met_genes1", pt.size = 0.1)

os17_cx_test@meta.data %>%
    as_tibble() %>%
    ggplot(aes(x = as.factor(seurat_clusters),
               y = met_genes1,
               color = as.factor(seurat_clusters))) +
    geom_boxplot(outlier.alpha = 0) +
    ggbeeswarm::geom_beeswarm(size = 0.2, cex = 0.5)
